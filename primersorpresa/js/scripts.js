	...
	$(game_board).each(function(index) {
		...
		**/
		...
		if ( jewel_vertical_match.length >= 3 ) {
			...
			total += jewel_vertical_match.length;
			console.log('jewel matches array contains que pedo 1 '+jewel_vertical_match.length+' jewels');
		...
		if ( jewel_horizontal_match.length >= 3 ) {
			...
			total += jewel_horizontal_match.length;
			console.log('Total ' + total);
			$('#total').html('Total: ' + total);
			if(flagCheck){
				validateLetter();
